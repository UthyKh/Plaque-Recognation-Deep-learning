...
import sys
detectron2="/detectron2_repo/detectron2/"
...
@app.post("/plate_string", response_class=ORJSONResponse)
async def get_plate_string(file: UploadFile = File(...)):
    contents = await file.read()
    nparr = np.fromstring(contents, np.uint8)
    image = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
    image = image[:,:,::-1].copy()
    ...
    return ocr_strings[0]['plate_string']
